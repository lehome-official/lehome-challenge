...
# ===== Advanced Example: Scripted/Rule-Based Policy =====
@PolicyRegistry.register("scripted")
class ScriptedPolicy(BasePolicy):
    """
    Scripted/Rule-based Policy Example (For Debugging).
    
    This policy does not use machine learning models but generates actions based on simple rules.
    It can be used for:
    1. Verifying if the environment is working correctly.
    2. Serving as a baseline for comparison.
    ...
    def select_action(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        Example Rule: 
        - First 50 steps: Stay still (zero action).
        - Afterwards: Execute a sine wave action (for demonstration).
        """
        ...
        # Simple Rule: Static for first 50 steps, then periodic motion
        if self.step_count < 50:
            # Hold current position (zero velocity/delta)
            action = np.zeros(action_dim, dtype=np.float32)
        else:
            # Execute small amplitude sine wave
            phase = (self.step_count - 50) * 0.1
            action = np.sin(phase + np.arange(action_dim)) * 0.02
